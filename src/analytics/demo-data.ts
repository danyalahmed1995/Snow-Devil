...
const repositories: AnalyticsRepository[] = [
  { id: 'nova-labs/snow-devil', nameWithOwner: 'nova-labs/snow-devil', ownerLogin: 'nova-labs', viewerPermission: 'ADMIN', defaultBranch: 'main', releaseMatching: true, deploymentMatching: true },
  { id: 'nova-labs/edge-runtime', nameWithOwner: 'nova-labs/edge-runtime', ownerLogin: 'nova-labs', viewerPermission: 'MAINTAIN', defaultBranch: 'main', releaseMatching: true, deploymentMatching: true },
  { id: 'nova-labs/data-pipeline', nameWithOwner: 'nova-labs/data-pipeline', ownerLogin: 'nova-labs', viewerPermission: 'WRITE', defaultBranch: 'trunk', releaseMatching: false, deploymentMatching: true, capabilityNote: 'Deployments are tracked; releases are not used.' },
  { id: 'nova-labs/old-prototype', nameWithOwner: 'nova-labs/old-prototype', ownerLogin: 'nova-labs', viewerPermission: 'READ', defaultBranch: 'main', archived: false, releaseMatching: false, deploymentMatching: false, capabilityNote: 'No release or deployment evidence is available.' },
];
...
export function createDemoAnalyticsDataset(): AnalyticsDataset {
  ...
  const current: DeliveryEntity[] = [
    { id: 'focus:review', repositoryId: repositories[0].id, type: 'pull_request', number: 4213, title: 'Implement offline analytics cache', stage: 'review', state: 'open', author: 'snowdevil-demo', createdAt: at(8), updatedAt: at(6), prOpenedAt: at(8), firstCommitAt: at(7), reviewState: 'requested', checkState: 'success', requestedReviewers: ['octo-reviewer'], sourceCompleteness: 'complete', evidence: ['Review requested from octo-reviewer'] },
    ...
    { id: 'focus:dependabot', repositoryId: repositories[0].id, type: 'pull_request', number: 4214, title: 'Bump vite from 7.3.4 to 7.3.5', stage: 'pull_requests', state: 'open', author: 'dependabot[bot]', isBot: true, createdAt: at(2), updatedAt: at(1), prOpenedAt: at(2), reviewState: 'none', checkState: 'success', sourceCompleteness: 'complete', evidence: ['Dependency update authored by Dependabot'] },
  ...
  entities.push({ id: 'inventory:stale-branch', repositoryId: repositories[3].id, type: 'branch', title: 'experiment/old-renderer', branchName: 'experiment/old-renderer', stage: 'coding', state: 'active', author: 'snowdevil-demo', createdAt: at(48), updatedAt: at(45), sourceCompleteness: 'partial', evidence: ['First unique commit observed 48 days ago', 'No merge or deletion evidence'] });
