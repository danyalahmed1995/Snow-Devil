...
export function CommandPalette() {
  ...
  return <div className="palette-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) close(); }}><section className="command-palette glass-panel-strong" role="dialog" aria-modal="true" aria-label="Search and commands" onKeyDown={onKeyDown}>
    ...
    <div id="palette-results" role="listbox" aria-label={`${results.length} results`} className="palette-results">{results.length ? results.map((result,index) => <button key={result.id} role="option" aria-selected={active === index} className={active === index ? 'is-active' : ''} onMouseEnter={() => setActive(index)} onClick={() => run(result,'current')}><span className="palette-result__icon">{iconFor(result.type)}</span><span className="palette-result__copy"><strong>{result.title}</strong><small>{result.repository && result.repository !== result.title ? `${result.repository} · ` : ''}{result.subtitle}</small></span><span className="palette-result__type">{result.type}</span>{active === index && <kbd>↵</kbd>}</button>) : <div className="palette-empty"><strong>{status && !status.startsWith('Local cache') ? 'Searching…' : 'No matching results'}</strong><span>{status && !status.startsWith('Local cache') ? 'Gathering resources from local cache.' : 'Unknown filters stay in the fuzzy query. Try a broader term.'}</span></div>}</div>
