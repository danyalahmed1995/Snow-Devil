...
export function CIWatcherPanel({ repositoryId, compact = false, testId }: { repositoryId?: string; compact?: boolean; testId?: string }) {
  ...
  const collapsedLimit = compact ? 3 : 7;
  const shown = runs.slice(0, expanded ? 20 : collapsedLimit);
  ...
  const accountStates = Object.values(repositoryState);
  const visibleState = scopedState ?? (runs.length === 0
    ? accountStates.find(state => state.status === 'loading' || state.status === 'refreshing')
      ?? accountStates.find(state => state.status !== 'ready')
      ?? (accountStates.length === 0 ? { status: 'loading' as const, message: 'Preparing the first CI snapshot' } : undefined)
    : accountStates.find(state => state.status === 'refreshing'));
  ...
  return <section className={`ci-watcher${compact ? ' ci-watcher--compact' : ''}${expanded ? ' is-expanded' : ''}`} aria-label={repositoryId ? `CI Watcher for ${repositoryId}` : 'Account CI Watcher'} data-testid={testId ?? (repositoryId ? 'repository-ci-watch' : 'account-ci-watch')}>
    ...
    {visibleState && visibleState.status !== 'ready' && runs.length === 0 && <div className="ci-watcher-state"><strong>{visibleState.status.replace(/_/g, ' ')}</strong><span>{visibleState.message ?? 'Workflow runs are loading.'}</span></div>}
    {visibleState && visibleState.status !== 'ready' && runs.length > 0 && <div className="ci-watcher-refresh-state" role="status">{visibleState.message ?? `${visibleState.status.replace(/_/g, ' ')} · Displaying the previous snapshot`}</div>}
    <div className="ci-watcher-list">{shown.map(run => <button key={run.id} className={`ci-run ci-run--${run.conclusion ?? run.status}`} onClick={() => open(run)} data-tooltip={`${run.workflowName} #${run.runNumber}\n${run.repositoryId} · Open the canonical GitHub Actions run in Snow Devil.`}><span className="ci-run-icon">{run.status !== 'completed' ? <CircleDotDashed size={14}/> : run.conclusion === 'success' ? <CheckCircle2 size={14}/> : <XCircle size={14}/>}</span><span><strong>{run.workflowName} <small>#{run.runNumber}</small></strong><small>{run.repositoryId} · {run.pullRequestNumber ? `PR #${run.pullRequestNumber}` : run.branch ?? 'branch unavailable'} · {relative(run.updatedAt)}</small></span><em>{outcome(run)}<small>{duration(run.durationMs)}</small></em><ExternalLink size={11}/></button>)}{shown.length === 0 && (!visibleState || visibleState.status === 'ready') && <div className="ci-watcher-state"><strong>No recent workflow runs</strong><span>GitHub returned no Actions runs for this scope.</span></div>}</div>
    {runs.length > collapsedLimit && <button className="ci-watcher-more" aria-expanded={expanded} data-tooltip={`${expanded ? 'Show less' : 'View all'} CI runs\n${expanded ? 'Return to the compact three-row status strip.' : 'Expand this panel to inspect up to 20 recent workflow runs.'}`} onClick={() => setExpanded(value => !value)}>{expanded ? 'Show less' : `View all ${runs.length}`}</button>}
  </section>;
}
