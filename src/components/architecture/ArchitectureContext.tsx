...
function MetricCard({ label, value, detail, icon }: { label: string; value: React.ReactNode; detail: string; icon: React.ReactNode }) {
  return <article className="architecture-metric"><div className="architecture-metric__icon">{icon}</div><div><span>{label}</span><strong>{value}</strong><small>{detail}</small></div></article>;
}
