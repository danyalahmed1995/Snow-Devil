...
export function Inspector() {
  ...
  return <div className="inspector"><div className="inspector-header"><h3>Inspector</h3></div><div className="inspector-content">{content}{isAnalytics && flowState.selectedAnalyticsEntity?.repositoryId && <div className="inspector-actions"><button type="button" onClick={() => { const repository = flowState.selectedAnalyticsEntity!.repositoryId!; useFlowStore.getState().setTabState('native:flow', { scope: 'repository', selectedRepository: { id: repository, nameWithOwner: repository } }); openNativeTab('native:flow', 'flow', 'Flow', false, true); }}>Open in Flow</button>{flowState.selectedAnalyticsEntity.kind === 'ci_health' && <><button type="button" onClick={() => { const repository = flowState.selectedAnalyticsEntity!.repositoryId!; useFlowStore.getState().setTabState('native:repository-simulator', { selectedRepository: { id: repository, nameWithOwner: repository } }); openNativeTab('native:repository-simulator', 'repositorySimulator', 'Repository Simulator', false, true); }}>Repository Simulator</button><button type="button" onClick={() => openNativeTab('native:ci-health', 'ciHealth', 'CI Health', false, true)}>CI Health</button></>}</div>}{target && <div className="inspector-actions"><button className="open-link inspector-open-tab" type="button" onClick={() => openBrowserTab(target.id, target.kind, target.title, target.url, false, true)}>Open in Tab</button><button type="button" onClick={() => void openInDefaultBrowser(target.url).then(() => setCopyStatus('Opened in default browser')).catch(error => setCopyStatus(error instanceof Error ? error.message : 'Open unavailable'))}><ExternalLink size={12} /> Open in Default Browser</button><button type="button" onClick={() => void copyCanonicalLink(target.url).then(() => setCopyStatus('Link copied')).catch(error => setCopyStatus(error instanceof Error ? error.message : 'Copy unavailable'))}><Copy size={12} /> Copy Link</button></div>}{demoUnavailableTarget && <button className="open-link inspector-open-tab" type="button" disabled>Open in Tab unavailable in Demo Mode</button>}<span className="inspector-copy-status" aria-live="polite">{copyStatus}</span></div></div>;
}
