...
function AnalyticsDetails({ tab }: { tab: InspectorTab }) {
  ...
  return <div className={`inspector-details inspector-details--${tab}`}>
    ...
    <section className="inspector-section"><h5 className="section-title">{isRisk ? 'Summary' : 'Why it appears'}</h5>{isRisk && <div className="metadata"><Meta label="Work item">{selected.title}</Meta><Meta label="Repository">{selected.repositoryId ?? 'Unknown'}</Meta><Meta label="Entity type">{selected.entityType?.replace(/_/g, ' ') ?? 'Unknown'}</Meta><Meta label="Current state">{selected.state ?? 'Unknown'}</Meta><Meta label="Primary risk">{selected.riskCategory?.replace(/_/g, ' ') ?? 'Unknown'}</Meta><Meta label="Secondary risks">{selected.secondaryRisks?.map(value => value.replace(/_/g, ' ')).join(', ') || 'None'}</Meta><Meta label="Age in risk state">{selected.riskAgeDays == null ? 'Unknown' : `${Math.ceil(selected.riskAgeDays)} business days`}</Meta>{selected.occurredAt && <Meta label="Last meaningful activity">{new Date(selected.occurredAt).toLocaleString()} · {selected.lastActivityLabel ?? 'activity'}{selected.lastActivityActor ? ` · ${selected.lastActivityActor}` : ''}</Meta>}<Meta label="Relevant owner">{selected.owner ?? 'Unassigned'}</Meta><Meta label="Confidence">{selected.confidence ?? 'unknown'}</Meta></div>}<h5 className="section-title inspector-subtitle">Why it is here</h5><p className="meta-val">{selected.reason ?? 'Selected from delivery analytics.'}</p>{!isRisk && <div className="metadata">{selected.confidence && <Meta label="Confidence">{selected.confidence}</Meta>}{selected.coverage && <Meta label="Coverage">{selected.coverage}</Meta>}{selected.sampleCount != null && <Meta label="Qualifying samples">{selected.sampleCount}</Meta>}{selected.excludedCount != null && <Meta label="Excluded / incomplete">{selected.excludedCount}</Meta>}{selected.occurredAt && <Meta label="Last activity">{new Date(selected.occurredAt).toLocaleString()}</Meta>}{selected.relatedEntityIds && <Meta label="Related entities">{selected.relatedEntityIds.length}</Meta>}</div>}{selected.definition && !isRisk && <p className="inspector-partial">{selected.definition}</p>}</section>
