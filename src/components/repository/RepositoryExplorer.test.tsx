...
const renderExplorer = (repository: string, initialPath?: string) => {
  const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } });
  return render(
    <QueryClientProvider client={queryClient}>
      <RepositoryExplorer repository={repository} initialPath={initialPath} />
    </QueryClientProvider>
  );
};
...
describe('RepositoryExplorer tree presentation', () => {
  ...
  it('centers the large-file guard inside the preview content area', async () => {
    renderExplorer('octo/repo', 'Benchmark Files/heavy_mdx_5mb_examples/01-large-doc.mdx');
  ...
  it('normalizes encoded spaces, punctuation, and unicode breadcrumb paths', async () => {
    renderExplorer('octo/repo', 'Benchmark%20Files/url-sensitive/100%25%20ready%20%231%3F.md');
    ...
    useModeStore.setState({ mode: 'demo', demoRevision: 0 });
    useExplorerStore.setState({ states: {} });
    renderExplorer('octo/repo', 'Unicode/%E9%9B%AA.md');
  ...
  it('normalizes live file payloads that omit byteSize', async () => {
    ...
    renderExplorer('octo/repo', 'artifact.bin');
