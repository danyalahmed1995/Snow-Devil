  ...
  return <AnalyticsPage title="Personal Focus" description="Your current work, aging signals, and evidence-backed actions" demo={analytics.mode === 'demo'} controls={<RefreshButton refreshing={analytics.isFetching} onClick={() => void analytics.refetch()} />}>
    ...
    {dataset && <>
      ...
      <div className="analytics-focus-grid">
        <SectionCard title="Work in Progress"><div className="analytics-wip-meter"><p><span>Current active items</span><strong>{active.length}</strong></p><div className="analytics-wip-track"><span style={{ width: `${Math.min(100, active.length / Math.max(1, usualWip * 2) * 100)}%`, background: aboveNormal ? undefined : '#3fb950' }} /></div><p><span>Your usual concurrent workload</span><strong>{usualWip}</strong></p>{aboveNormal && <p style={{ color: '#ff7b72' }}><AlertTriangle size={11} /> Current WIP is meaningfully above your historical norm.</p>}</div><div className="analytics-list">{withAge.slice(0, 6).map(item => <button key={item.entity.id} type="button" onClick={() => selectEntity(item.entity, `${Math.ceil(item.age)} business days in ${item.entity.stage}`)}><span><strong>{item.entity.title}</strong><small>{item.entity.repositoryId} | {item.entity.stage}</small></span><time>{Math.ceil(item.age)}d</time></button>)}</div></SectionCard>
        <SectionCard title="Aging Items"><div className="analytics-list">{withAge.filter(item => item.age >= settings.inventoryThresholds.agingDays).slice(0, 7).map(item => <button key={item.entity.id} type="button" onClick={() => selectEntity(item.entity, item.age >= settings.inventoryThresholds.staleDays ? 'Beyond personal P90 age threshold' : 'Beyond fixed aging threshold')}><span><strong>{item.entity.title}</strong><small>{item.age >= settings.inventoryThresholds.staleDays ? 'Beyond personal P90' : 'Above normal stage age'} | {item.entity.repositoryId}</small></span><time style={{ color: item.age >= settings.inventoryThresholds.staleDays ? '#ff7b72' : '#e3b341' }}>{Math.ceil(item.age)}d</time></button>)}</div></SectionCard>
      ...
      <div className="analytics-focus-tip"><Star size={17} color="#e3b341" /><span><strong>Focus Tip</strong>{tip}</span></div>
    </>}
  </AnalyticsPage>;
}
