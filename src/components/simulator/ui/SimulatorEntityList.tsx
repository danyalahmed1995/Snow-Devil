...
export function SimulatorEntityList({ entities, selectedId, onSelect, title = 'Entities', emptyLabel = 'No matching entities.', query, onQueryChange, scrollRef, revealId, flashId, registerRow }: { entities: SimulatorEntityState[]; selectedId?: string; onSelect: (id: string) => void; title?: string; emptyLabel?: string; query: string; onQueryChange: (value: string) => void; scrollRef?: RefObject<HTMLDivElement | null>; revealId?: string; flashId?: string; registerRow?: (key: string, node: HTMLButtonElement | null) => void }) {
    ...
    <section className="simulator-panel simulator-entities">
      ...
      <div ref={scrollRef} className="simulator-panel__scroll">
        {filtered.length === 0 ? <div className="simulator-empty">{emptyLabel}</div> : visible.map(entity => (
          <button type="button" key={entity.id} ref={node => registerRow?.(entity.id, node)} data-entity-id={entity.id} data-history-target-key={entity.id} className={`simulator-entity-row${selectedId === entity.id ? " is-selected" : ""}${flashId === entity.id ? ' is-revealed' : ''}`} onClick={() => onSelect(entity.id)} data-tooltip={`${formatEntityReference(entity)} ${formatEntityTitle(entity)}\n${entity.repositoryId} · Select to inspect this item.`}>
