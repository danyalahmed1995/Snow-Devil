    ...
    for label in labels {
        if let Some(webview) = app.get_webview(&label) {
            webview
                ...
                .map_err(|e| format!("Failed to clear embedded browsing data: {e}"))?;
            let _ = webview.close();
        }
    }
    if let Some(main) = app.get_webview("main") {
        main.clear_all_browsing_data()
            .map_err(|e| format!("Failed to clear app webview data: {e}"))?;
    }
    Ok(())
}
...
mod tests {
    ...
    fn reset_sql_is_idempotent_by_construction() {
        ...
        conn.execute_batch(
            "CREATE TABLE cache (id INTEGER); DELETE FROM cache; DELETE FROM cache;",
        )
        .unwrap();
    }
