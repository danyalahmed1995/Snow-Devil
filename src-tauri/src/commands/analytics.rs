...
pub async fn analytics_fetch_rest(endpoint: String) -> Result<AnalyticsApiResponse, String> {
    ...
    let response = reqwest::Client::new().get(format!("https://api.github.com{}", endpoint))
        .bearer_auth(token).header("User-Agent", "snow-devil-analytics")
        .header("Accept", "application/vnd.github+json").send().await.map_err(|e| e.to_string())?;
    let status = response.status().as_u16();
    let rate_remaining = response.headers().get("x-ratelimit-remaining").and_then(|v| v.to_str().ok()).and_then(|v| v.parse().ok());
    let rate_reset = response.headers().get("x-ratelimit-reset").and_then(|v| v.to_str().ok()).and_then(|v| v.parse().ok());
    let next_page = response.headers().get("link").and_then(|v| v.to_str().ok()).and_then(parse_next_page);
    let body = response.json::<Value>().await.unwrap_or(Value::Null);
    Ok(AnalyticsApiResponse { status, body, next_page, rate_remaining, rate_reset })
}

fn parse_next_page(link: &str) -> Option<u32> {
    link.split(',').find(|part| part.contains("rel=\"next\""))
        .and_then(|part| part.split('<').nth(1))
        .and_then(|part| part.split('>').next())
        .and_then(|value| url::Url::parse(value).ok())
        .and_then(|url| url.query_pairs().find(|(key, _)| key == "page").and_then(|(_, value)| value.parse().ok()))
}
...
mod tests {
    ...
    fn parses_next_page_without_following_untrusted_urls() {
        ...
        assert_eq!(parse_next_page("<https://api.github.com/user/repos?page=1>; rel=\"prev\""), None);
    }
}

#[tauri::command]
pub fn save_analytics_records(state: State<'_, AppState>, records: Vec<AnalyticsRecord>) -> Result<(), String> {
    ...
    let tx = conn.transaction().map_err(|e| e.to_string())?;
    {
        ...
        for record in records { statement.execute(rusqlite::params![record.account_login, record.repository_id, record.source_type, record.source_id, record.updated_at, record.payload_json]).map_err(|e| e.to_string())?; }
    }
    tx.commit().map_err(|e| e.to_string())
}

#[tauri::command]
pub fn get_analytics_records(state: State<'_, AppState>, account_login: String) -> Result<Vec<AnalyticsRecord>, String> {
    ...
    let rows = stmt.query_map([account_login], |row| Ok(AnalyticsRecord { account_login:row.get(0)?,repository_id:row.get(1)?,source_type:row.get(2)?,source_id:row.get(3)?,updated_at:row.get(4)?,payload_json:row.get(5)? })).map_err(|e| e.to_string())?;
    rows.collect::<Result<Vec<_>, _>>().map_err(|e| e.to_string())
}

#[tauri::command]
pub fn save_analytics_sync_state(state: State<'_, AppState>, value: AnalyticsSyncState) -> Result<(), String> {
    let mut guard = state.db_conn.lock().map_err(|e| e.to_string())?;
    let conn = guard.as_mut().ok_or("Database connection not found")?;
...
#[tauri::command]
pub fn get_analytics_sync_state(state: State<'_, AppState>, account_login: String) -> Result<Option<AnalyticsSyncState>, String> {
    let guard = state.db_conn.lock().map_err(|e| e.to_string())?;
    let conn = guard.as_ref().ok_or("Database connection not found")?;
    ...
    let mut rows = stmt.query_map([account_login], |row| Ok(AnalyticsSyncState { account_login:row.get(0)?,status:row.get(1)?,current_stage:row.get(2)?,current_repository:row.get(3)?,completed_repositories_json:row.get(4)?,failed_repositories_json:row.get(5)?,continuation_json:row.get(6)?,last_attempted_at:row.get(7)?,last_successful_at:row.get(8)?,retention_start:row.get(9)?,coverage_start:row.get(10)?,coverage_end:row.get(11)?,counts_json:row.get(12)?,rate_limit_json:row.get(13)?,error:row.get(14)?,settings_fingerprint:row.get(15)? })).map_err(|e| e.to_string())?;
